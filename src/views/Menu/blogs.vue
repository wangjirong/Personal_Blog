        ...
        <ul class="flex-column">
          <li v-for="(blog,index) in blogList" :key="index">
            <h3 class="title">
              ...
              <div class="date">
                <span>{{blog.date}}</span>
                <span>{{blog.yearMonth}}</span>
              </div>
            </h3>
            <div class="blogImgState">
...
export default {
  ...
  methods: {
    async getAll() {
      ...
      //全部文章
      this.allBlog = this.handleBlogList(res.data.allBlogs);
      this.blogList = this.allBlog;
      //top5
      this.hotTopBlogList = this.handleBlogList(res.data.hotTop5);
      //置顶3篇
      this.recommendTopBlogList = this.handleBlogList(res.data.recommendTop3);
      //最近访客
    ...
    async searchByTag(keyword) {
      const res = await this.searchByKeyword(keyword);
      this.blogList = res;
      await this.handleBlogList();
    },
    //根据关键字进行搜索
    async searchByKeyword(keyword) {
      ...
      if (res.data.length === 0) {
        Message.error("抱歉！没有找到，请换个关键词！！！");
        Message.success("3秒后返回....");
        setTimeout(() => {
          return this.allBlog;
        }, 3000);
      } else return this.handleBlogList(res.data);
    },
    //对文章时间进行初始规范化
    handleBlogList(arrList) {
      arrList.forEach(blog => {

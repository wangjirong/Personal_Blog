  ...
  methods: {
    getDetailBlog() {
      ...
      this.$axios
        ...
        .then(res => {
          this.blog = res.data;
          let Time = new Date(res.data.date);
          ...
          this.fullTime =
            getYearMonDay(res.data.date) + " " + getDetailTime(res.data.date);

          this.justDay = this.getfullValue(day);
          this.justYearMon =
            this.getfullValue(year) + "年" + this.getfullValue(month) + "月";
          //解析md文章---改未直接编辑器展示
          // this.context = marked(this.blog.md, { sanitize: true });
        })
        .catch(error => {
          throw error;
        });
    ...
    async getAllComments() {
      ...
      Comment.forEach(comment => {
        comment.yearMonDay = getYearMonDay(comment.date);
        comment.detailSeconds = getDetailTime(comment.date);
      });
...
@media screen and (max-width: 500px) {
  #detailBlog {
    .container {
      background: url("../../assets/img/mobile/mobile_bg5.jpg") fixed center
        center no-repeat;
      background-size: 100% 100%;
      .box {
        margin: 1rem auto;
